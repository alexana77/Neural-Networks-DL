...
#         print(a.shape, y_pred.shape. x.shape, a.shape, parameters.shape)
        a_next, yt_pred, cache = rnn_cell_forward(x[:,:,t], a_next, parameters)
        # Save the a_next of the new "next" hidden state in a (≈1 line)
        a[:,:,t] = a_next
...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    da_prev = np.dot(parameters['Wf'].T,dft)+ np.dot(parameters['Wi'].T,dit)+ np.dot(parameters['Wc'].T,dcct)+ np.dot(
        parameters['Wo'].T,dot)
    dc_prev = dc_next*dft+dot*(1-np.tanh(c_next)**2)*dot*da_next
    dxt =   np.dot(parameters['Wf'].T,dft)+ np.dot(parameters['Wi'].T,dit)+ np.dot(parameters['Wc'].T,dcct)+ np.dot(
        parameters['Wo'].T,dot)
    ### END CODE HERE ###
...
def lstm_backward(da, caches):
    ...
    for t in reversed(range(T_x)):
        print('n_x', n_x, 'n_a',n_a,'m', m,'T_x', T_x)
        print('da shape', da.shape)
        
        # Compute all gradients using lstm_cell_backward. Choose wisely the "da_next" (same as done for Ex 6).
        print(da[:n_a,:,t].shape, da[n_a:,:,t].shape)
        gradients = lstm_cell_backward(da[:,:,t-1], da[:,:,t], caches[t])
        print('dc_prev shape', gradients["dc_prev"].shape)
        print(gradients["dxt"].shape)
        # Store or add the gradient to the parameters' previous step's gradient
        da_prevt = gradients["da_prev"]
        dc_prevt = gradients["dc_prev"]
        dx[:,:,t] = gradients["dxt"][n_a:,:]
        dWf += gradients["dWf"]
        dWi += gradients["dWi"]
        dWc += gradients["dWc"]
        dWo += gradients["dWo"]
